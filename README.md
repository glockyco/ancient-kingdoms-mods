...
| Mod                | Summary                                                                                                                                                                                                                                             |
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `CombatVerification` | Registers typed HotRepl commands for checking, building, and measuring a combat fixture against the running game: `fixture.validate` (needs the world loaded, because the game's class definitions arrive with it), `fixture.createCharacter` (needs character selection open), `fixture.buildCharacter` (brings a newly created character to a declared level, attribute allocation, skill levels, equipment, and companions), and `probe.statSheet` (reads the complete combat state of the player and its companions). Used by the combat verification harness, not during play. |
