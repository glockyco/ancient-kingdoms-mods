...
| `BossSkillTracker`  | In-combat HUD for boss, elite, and fabled monster ability cooldowns, grouped by enemy with icon cooldown bars and the monster's own shared special-cast deadline, read from the server it hosts. The gate readout spans that deadline and the basic attack cycle the monster must finish before it can select a skill. |
