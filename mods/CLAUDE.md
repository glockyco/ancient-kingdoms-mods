...
Game types from `Assembly-CSharp` are accessed via the `Il2Cpp.*` namespace — **never** as bare type names. Mirror networking types use the `Mirror.*` namespace.

```csharp
// Game types — always Il2Cpp. prefix
Il2Cpp.UILogin, Il2Cpp.UICharacterSelection, Il2Cpp.NetworkManagerMMO
Il2Cpp.UIServerList, Il2Cpp.NetworkState

// Mirror types — Il2CppMirror. prefix (NOT Mirror.)
Il2CppMirror.NetworkClient.localPlayer
Il2CppMirror.NetworkManager.singleton

// Add using to avoid repetition
using Il2Cpp;  // then use bare name: UILogin, UICharacterSelection, etc.
...
`Assembly-CSharp.dll` in `Il2CppAssemblies/` is a near-empty stub. Real type wrappers are generated by Cpp2IL → Il2CppInterop and live in the `Il2Cpp.*` namespace.
...
## Building on macOS via CrossOver
