...
| `probe.perHitDamage`      | job  | Records every hit the local player lands inside a window, each with the health it took, the victim and a server timestamp, and derives the action count from the same window so a miss can be told from a landing. Something else has to drive the actions; the probe only listens. Each hit names the skill the engine chose, the school, and the amount the caster asked for before the engine's own steps. Reports the fidelity tier reached and the seed it used. Args: `{"windowSeconds": number, "seed": int?}`; an absent seed is taken from the clock and still reported, and an identical seed does not reproduce the sequence. |
