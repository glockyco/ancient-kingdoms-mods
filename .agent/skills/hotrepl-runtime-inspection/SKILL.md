...
| `probe.actionInterval`     | job  | Reads how often the local player can act. Stills its attack loop first and reports every value it cleared, then samples for `windowSeconds` and reports the actions that completed inside the window with the gaps between them. Reports the period the engine enforces, the haste, and each worn weapon with its delay. Args: `{"windowSeconds": number?}`; absent or zero takes the reading without watching for actions. |
