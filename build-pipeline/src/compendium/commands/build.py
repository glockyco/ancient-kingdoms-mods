...
def create_database(db_path: Path, schema_path: Path) -> sqlite3.Connection:
    ...
    if db_path.exists():
        db_path.unlink()
        console.print(f"  Removed existing database")
    ...
    console.print(f"  [green]OK[/green] Database created with schema")
    return conn
...
def load_summon_triggers(conn: sqlite3.Connection, export_dir: Path) -> None:
    ...
    console.print(f"  [green]OK[/green] Loaded {placeholder_count} placeholder relationships")
...
def denormalize_data(conn: sqlite3.Connection) -> None:
    ...
    dropped_by = {}  # item_id -> [{"monster_id": ..., "rate": ..., "zone_id": ...}]
    ...
    gathered_from = {}  # item_id -> [{"gather_item_id": ..., "rate": ...}]
    ...
    sold_by = {}  # item_id -> [{"npc_id": ..., "price": ..., "currency_item_id": ..., "zone_id": ...}]
    ...
    crafted_from = {}  # item_id -> [{"recipe_id": ..., "result_amount": ...}]
    ...
    used_in_recipes = {}  # item_id -> [{"recipe_id": ..., "amount": ...}]
    ...
    needed_for_quests = {}  # item_id -> [{"quest_id": ..., "purpose": ..., "amount": ...}]
    ...
    granted_by_items = {}  # skill_id -> [{"item_id": ..., "type": ..., ...}]
    ...
    for item_id, recipes in used_in_recipes.items():
        cursor.execute(
            "UPDATE items SET used_in_recipes = ? WHERE id = ?",
            (json.dumps(recipes), item_id)
        )

    for item_id, quests in needed_for_quests.items():
        cursor.execute(
    ...
    console.print(f"  [green]OK[/green] Updated {len(dropped_by)} items with monster drops")
    console.print(f"  [green]OK[/green] Updated {len(sold_by)} items with vendor info")
    console.print(f"  [green]OK[/green] Updated {len(rewarded_by)} items as quest rewards")
    console.print(f"  [green]OK[/green] Updated {len(crafted_from)} items with crafting recipes")
    console.print(f"  [green]OK[/green] Updated {len(gathered_from)} items with gather sources")
    console.print(f"  [green]OK[/green] Updated {len(used_in_recipes)} items used in recipes")
    console.print(f"  [green]OK[/green] Updated {len(needed_for_quests)} items needed for quests")
    console.print(f"  [green]OK[/green] Updated {len(granted_by_items)} skills granted by items")
