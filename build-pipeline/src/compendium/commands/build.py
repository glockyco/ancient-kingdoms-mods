...
def denormalize_data(conn: sqlite3.Connection) -> None:
    ...
    console.print(
        "  Denormalizing chest reward item names and calculating exact drop chances..."
    )
    cursor.execute("""
    ...
    def calculate_exact_chest_probabilities(
    ...
    for chest_id, chest_rewards_json, num_items in cursor.fetchall():
        ...
        if updated_rewards and num_items > 0:
            exact_probs = calculate_exact_chest_probabilities(
    ...
    console.print(
        f"  [green]OK[/green] Updated {potion_buff_count} items with potion buff names, {food_buff_count} items with food buff names, {relic_buff_count} items with relic buff names, {scroll_skill_count} items with scroll skill names, {pack_final_count} pack items with final item names"
    )
    console.print(
