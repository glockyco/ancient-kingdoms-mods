...
from compendium.utils.paths import get_repo_root
...
def load_redactions(config_path: Path | None = None) -> RedactionConfig:
    ...
    Raises `FileNotFoundError` when the file is absent. An empty configuration
    would publish every unreleased zone, every named entity, and every withheld
    position, and the build would report success. The file is required.
    """
    if config_path is None:
        # Ask for the repository root rather than counting directories upwards.
        # A count is silently wrong the moment this module moves, and the file
        # it fails to find is the one that configures every redaction.
        config_path = get_repo_root() / "redactions.toml"

    if not config_path.exists():
        raise FileNotFoundError(
            f"Required redaction configuration is unavailable: {config_path}. "
            "Building without it would publish content that redaction removes."
        )
