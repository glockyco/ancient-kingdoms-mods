...
- Publish an item tooltip that describes the item: render it, then remove the decoration that answers a question about the exporting character.
...
- Capture a visual asset from a source that does not depend on which way an actor is facing.
- **BREAKING** for published values: 240 item tooltips lose colour markup, 14 gather item names change, 12 spawn positions move by up to 2.4 units, and one published image is replaced.
...
- **Published data:** the values listed above. Each is a correction, and each is a one-time diff in the generated database, the search index and the map.
