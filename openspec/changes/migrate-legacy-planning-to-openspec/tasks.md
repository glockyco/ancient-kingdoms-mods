...
records can proceed while the prerequisite changes remain active. Do not remove the legacy index,
directory, or authority pointers until both prerequisite changes are archived. This change does not
implement feature work discovered during an audit.

## 1. Priority gate and inventory

- [x] 1.1 Confirm the state of `add-combat-verification-harness` and
      `add-gear-and-rotation-planner`. Both remain active. Permit audit and independently safe stale-record
      removal, but defer the final authority cutover until both changes are archived.
