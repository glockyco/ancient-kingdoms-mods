...
- [x] 12.1 Move the two quest identifiers from `[quests.exclude].ids` to `[entities.exclude].ids`, and confirm the closure removes both. The manual seed already searches the quests table. Measured: manual removals rise from 1 to 3, and the build now removes 60 identifiers against 57.
- [x] 12.2 Delete `_apply_quest_exclusions` and the `exclude_quest_ids` field. Confirm no reference to a removed quest survives, and note that `npcs.quests_offered` is rebuilt by a later denormalizer rather than scrubbed. Removed 41 lines. No column of any table names either quest. The closure also removed `items:key_to_the_north_wing`, which `relics_of_the_ancient_faith` gave as its reward: the hand-written pass deleted the quest and published the reward with no remaining source.
- [x] 12.3 Prove the ledger now records both quests with a mechanism and a reason, and that `redactions explain the_lost_warden` answers. Both are recorded as manual. `explain key_to_the_north_wing` reports a cascade at pass 2 followed from `quests:relics_of_the_ancient_faith`. Position suppression rose from 885 values to 887, because both quests carry `zone_id_final_npc = 23` and the old order deleted them before suppression ran.
- [x] 12.4 Report what hidden crafting removed to the ledger, per item, in the way position suppression reports per zone. Recorded: 8 items, one recipe each.
- [x] 12.5 Confirm the verification subject covers every mechanism. After 12.1, every entity removal runs through the closure, so all 60 are already covered. Record why the two attribute mechanisms add none: position suppression removes geometry, which carries no identifier, and hidden crafting removes recipes whose identifier is the identifier of the item they produce. Scanning those would report 7 items published on purpose, and nothing in the schema references a recipe table, so the removal can dangle nothing. `tests/test_redaction_config.py` pins both facts.
- [x] 12.5a Fail the build when `redactions.toml` cannot be read. Moving the module broke a path built by counting parent directories, and the loader answered with an empty configuration and a dim note, which would have published every unreleased zone and every withheld position while reporting success. The path now asks for the repository root, and an absent file raises.
- [x] 12.6 Move position suppression into the redaction package as `geometry.py`, and hidden crafting as `crafting.py`. The module named `exclusions` implemented suppression, which is the name of the other mechanism. The design proposed `sources.py`, and two modules already carry that name under `denormalizers`. `recipes.py` collides with `denormalizers/recipes.py` in the same import list, so the module takes the name of its configuration key, `hide_crafting`.
- [x] 12.7 Rename `compendium/redaction.py` to `redactions/config.py`, so the configuration no longer sits one letter from the package it configures.
- [x] 12.8 Update `redactions.toml` comments and `build-pipeline/CLAUDE.md` for the removed key and the moved modules. The CLAUDE.md section listed three keys and no ledger, and now names the two families, the five configuration keys, the modules, and the four commands. `docs/plans/2026-08-10-entity-artwork-pipeline.md` is a draft plan whose Defect C says redaction does not reach artwork, which this change fixed, and it cited a configuration switch that no longer exists. Its premise is corrected in place.
