#!/usr/bin/env python3
"""Check that the agent instruction surface obeys the rules the repository states about itself.

`scripts/check-agent-docs.sh` is the entry point. This module is importable so that
`scripts/check_agent_docs_test.py` can build a fixture surface and assert that each check catches the
violation it exists to catch. A check that has never failed is indistinguishable from one that cannot.
"""
...
def check(root: Path) -> tuple[list[str], dict[str, int]]:
    """Return every violation found under `root`, and the counts reported on success."""
    ...
    counts = {
        "AGENTS.md": len(agents),
        "skills": len(skills),
        "skill references": len(references),
        "rules": len(rules),
    }
...
def main(argv: list[str]) -> int:
    ...
    if errors:
        ...
        return 1
    print("agent-docs check passed: " + ", ".join(f"{n} {k}" for k, n in counts.items()))
    return 0
