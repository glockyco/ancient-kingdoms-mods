---
title: "Treasure Hunter Profession Page Implementation Plan"
type: plan
status: implemented
created: 2026-05-01
parent:
...
Earlier iterations included a paragraph below the summary cards explaining "+10 pp" and "the table focuses on relics". Skip that paragraph — the live `Relic chance bonus` tile already shows the bonus value, the `At selected skill` column header already names what the column does, and step 5 of How It Works already states that Treasure Hunter improves relic odds. The one targeted exception is a one-sentence footnote inside the table's bordered container that explains why `Change` values exceed the per-roll bonus shown in the tile: each reward rolls up to 10 times per chest (`server-scripts/ChestItem.cs:24-66`), so the per-roll bonus compounds into a larger per-chest gain. Connect the footnote to the column with a `<sup>*</sup>` marker on the `Change` header and a leading `*` (wrapped in `<span aria-hidden="true">` to keep it out of the screen-reader stream) on the footnote. Keep the footnote skill-agnostic — do not hardcode "+10 pp" because the per-roll bonus is `skillFraction * 0.1` and only equals 10 pp at 100%. Use `text-sm` so it stays readable.
