 ...
 *
 * Run automatically via the "prebuild" npm script. The output is committed so
 * clean checkouts can type-check without the generated database. Keep the
 * generated source Prettier-stable so prebuild never dirties an unchanged tree.
 */
...
const traps = count("SELECT COUNT(*) as count FROM traps");
const factions = count("SELECT COUNT(*) as count FROM factions");
...
const raw = `// AUTO-GENERATED by scripts/generate-home-counts.mjs — do not edit.
...
`;
const formatted = await format(raw, { filepath: outPath });
