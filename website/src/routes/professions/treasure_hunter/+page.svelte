...
<svelte:head>
  ...
  <meta
    name="description"
    content="{data.profession.description} View all treasure maps."
  />
</svelte:head>
        ...
        <tbody>
          {#each data.treasureMaps as map (map.id)}
            <tr class="border-t hover:bg-muted/30">
              ...
              <td class="p-3">
                <span
                  class="px-2 py-0.5 rounded text-xs font-medium text-white bg-quality-{map.quality}"
                >
                  {getQualityName(map.quality)}
                </span>
              </td>
            </tr>
          {/each}
