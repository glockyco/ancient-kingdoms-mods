...
<div class="container mx-auto max-w-5xl space-y-8 p-8">
  ...
  <Card.Root id="cycle" class="bg-muted/30">
    ...
    <Card.Content class="space-y-4 text-sm text-muted-foreground">
      ...
      <ol class="list-decimal space-y-1 pl-5">
        ...
        <li>
          <!-- Source: server-scripts/Monster.cs:879-886 — corpses with zero occupied inventory slots are removed from one second after death onward. -->
          A corpse that holds no items — because nothing dropped or everything was
          looted — disappears almost immediately instead.
        </li>
        <li>
          <!-- Source: server-scripts/Monster.cs:1857-1865 and 2486-2490 — hidden corpse is warped back to its start position. -->
        ...
        <li>
          <!-- Source: server-scripts/Monster.cs:1842-1854 — on respawn aggro, debuffs, gold, and loot state are cleared, then the monster is shown and revived. -->
  ...
  <Card.Root id="empty-zones" class="bg-muted/30">
    ...
    <Card.Content class="space-y-3 text-sm text-muted-foreground">
      <p>
        ...
        <!-- Source: server-scripts/Player.cs:3437, 10598, 12623 — zone cleanup runs 5 seconds after respawn, portal travel, or resurrection; NetworkManagerMMO.cs:718 and 820 — and on disconnect. -->
      ...
      <ul class="list-disc space-y-1 pl-5">
        <li>
          <!-- Source: server-scripts/Monster.cs:2689-2710 — on zone deactivation dead monsters are hidden, warped home, and their corpse deadline is set to now. -->
          Corpses are removed the moment the zone shuts down. Loot you left on a corpse
          is gone when you come back.
        </li>
        <li>
          <!-- Source: server-scripts/Monster.cs:2668-2687 — on zone reactivation living monsters warp to start position, heal to full, and clear target, aggro, pets, and debuffs. -->
        ...
        <li>
          <!-- Source: server-scripts/Monster.cs:888-895 and Entity.cs:221-229 — respawn deadlines are absolute server timestamps, but the state machine that acts on them only runs while the zone object is active. -->
  ...
  <Card.Root id="bosses" class="bg-muted/30">
    ...
    <Card.Content class="space-y-4">
      <ul class="list-disc space-y-1 pl-5 text-sm text-muted-foreground">
        ...
        <li>
          <!-- Source: server-scripts/NetworkManagerMMO.cs:111 and Monster.cs:564-585 — saved deadlines are loaded at server start; bosses whose deadline has not passed start hidden. -->
        ...
        <li>
          <!-- Source: server-scripts/Player.cs:13550-13554 — a dungeon renewal zeroes boss and elite deadlines, including the saved ones. -->
  ...
  <Card.Root id="rare-spawns" class="bg-muted/30">
    ...
    <Card.Content class="space-y-4">
      <p class="text-sm text-muted-foreground">
        <!-- Source: server-scripts/Monster.cs:2162-2167 — a failed spawn roll keeps the monster hidden and re-arms the timer for another full interval. -->
        <!-- Source: server-scripts/Monster.cs:659-664 — the same roll happens once at server start. -->
  ...
  <Card.Root id="renewal-sages" class="bg-muted/30">
    ...
    <Card.Content class="space-y-4">
      <ul class="list-disc space-y-1 pl-5 text-sm text-muted-foreground">
        <li>
          <!-- Source: server-scripts/Player.cs:13544-13555 — the renewal sets respawnTimeEnd to 0 for every respawn-enabled monster in the dungeon; boss and elite saved deadlines are zeroed too. -->
        ...
        <li>
          <!-- Source: server-scripts/Player.cs:13544-13555 — the renewal only writes timers; health and state of living monsters are untouched. -->
        ...
        <li>
          <!-- Source: server-scripts/Player.cs:13530-13536 — the renewal is refused while any player is inside the dungeon. -->
        ...
        <li>
          <!-- Source: server-scripts/Npc.cs:1693-1699 and UINpcTrading.cs:760-768 — the fee runs through the vendor purchase formula, discounted by Charisma up to 25%. -->
      ...
      <p class="text-sm text-muted-foreground">
        <!-- Source: server-scripts/Player.cs:13805-13817 and Monster.cs:2162-2167 — a zeroed deadline triggers the spawn roll on the next zone activation; a failed roll re-arms the full interval until the next renewal zeroes it again. -->
  ...
  <Card.Root id="spawn-windows" class="bg-muted/30">
    ...
    <Card.Content class="space-y-4">
      <p class="text-sm text-muted-foreground">
        ...
        minutes.
        <!-- Source: server-scripts/Monster.cs:1821-1824 — respawn is held while outside the spawn window. -->
        <!-- Source: server-scripts/Monster.cs:844-851 and 1297-1301 — outside its window a monster with no aggro is hidden and warped home. -->
      ...
      <p class="text-sm text-muted-foreground">
        <!-- Source: server-scripts/Monster.cs:1817-1820 — Halloween monsters additionally only respawn while the Halloween event is active. -->
  ...
  <Card.Root id="summons" class="bg-muted/30">
    ...
    <Card.Content class="space-y-4">
      <p class="text-sm text-muted-foreground">
        <!-- Source: server-scripts/Monster.cs:619-627 — summonable monsters start hidden; only chance-based non-elite summons start with their respawn timer armed. -->
        ...
        <!-- Source: server-scripts/Monster.cs:1809 and 1825-1840 — the spawn check requires the summon's own respawn timer elapsed plus all trigger monsters dead at the same time; a zone-wide message is broadcast on success. -->
      ...
      <p class="text-sm text-muted-foreground">
        <!-- Source: server-scripts/Monster.cs:2162-2167 — summons with a spawn chance below 100% roll on the check; a failed roll re-arms the timer for a full interval. -->
  ...
  <Card.Root id="leashing" class="bg-muted/30">
    ...
    <Card.Content>
      <ul class="list-disc space-y-1 pl-5 text-sm text-muted-foreground">
        <li>
          <!-- Source: server-scripts/Monster.cs:49 and 941-956 — chase limit is the per-monster follow distance, default 20 units, measured from the spawn point. -->
          <!-- Source: server-scripts/Monster.cs:1234-1249 — beyond the follow distance the monster drops its target, clears debuffs, and returns home with bonus sprint speed. -->
        ...
        <li>
          <!-- Source: server-scripts/Monster.cs:665-666 and 1249-1254 — the distance check is skipped entirely for non-boss, non-elite monsters in dungeon zones. -->
          <!-- Source: server-scripts/Monster.cs:1540-1548 — chasing ends when the target is no longer reachable by pathfinding. -->
        ...
        <li>
          <!-- Source: server-scripts/Monster.cs:2262 and Monster.cs:665-666 — while returning, a monster only re-engages targets within 80% of its follow distance; normal dungeon monsters always re-engage (flag set at Monster.cs:663-664). -->
        ...
        <li>
          <!-- Source: server-scripts/Monster.cs:1383-1406 — on arriving home the monster heals to full, restores mana, clears aggro and debuffs, and destroys its summoned pets. -->
  ...
  <Card.Root id="other-spawns" class="bg-muted/30">
    ...
    <Card.Content>
      <ul class="list-disc space-y-2 pl-5 text-sm text-muted-foreground">
        <li>
          <!-- Source: server-scripts/Monster.cs:2113-2123 — on death a monster can spawn a replacement at its corpse position with a configured probability; the replacement never respawns. -->
        ...
        <li>
          <!-- Source: server-scripts/Monster.cs:502-510 and 1817-1820 — Halloween monsters start hidden and only respawn while the seasonal event is active. -->
