<script lang="ts">
  ...
  const bestiaryClassName = $derived(
    data.monster.class_name && data.monster.class_name !== "Unknown"
      ? data.monster.class_name
      : null,
  );
  ...
  const skillColumns: ColumnDef<MonsterSkill>[] = [
    ...
    },
    {
      ...
      header: "Cooldown",
      accessorFn: (row) => {
        if (!row.cooldown) return 0;
        try {
          const parsed = JSON.parse(row.cooldown) as LinearValue;
    ...
    },
    {
      ...
      header: "Cast Time",
      accessorFn: (row) => {
        if (!row.cast_time) return 0;
        try {
          const parsed = JSON.parse(row.cast_time) as LinearValue;
          return parsed.base_value;
        } catch {
          return 0;
        }
...
<Seo
  title={`${data.monster.name} - Ancient Kingdoms`}
